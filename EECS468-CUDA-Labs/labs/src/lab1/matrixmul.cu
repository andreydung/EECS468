...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	Matrix M_gpu = AllocateDeviceMatrix(M);
	Matrix N_gpu = AllocateDeviceMatrix(N);
	Matrix P_gpu = AllocateDeviceMatrix(P);

	CopyToDeviceMatrix(M_gpu, M);
	CopyToDeviceMatrix(N_gpu, N);
